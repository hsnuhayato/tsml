...
public:
    ...
    inline bool is_inside_foot (const cnoid::Vector3& leg_pos, const bool is_lleg, const double margin = 0.0)
    {
    ...
    inline bool is_front_of_foot (const cnoid::Vector3& leg_pos, const double margin = 0.0)
    {
        return leg_pos(0) >= (leg_front_margin - margin);
    };
    inline bool is_rear_of_foot (const cnoid::Vector3& leg_pos, const double margin = 0.0)
    {
        return leg_pos(0) <= (-1 * leg_rear_margin + margin);
    };
    inline bool is_inside_support_polygon (Eigen::Vector2d& p, const cnoid::Vector3& offset = cnoid::Vector3::Zero(), const bool& truncate_p = false, const std::string& str = "")
    {
    ...
    void calc_convex_hull (const std::vector<std::vector<Eigen::Vector2d> >& vs, const std::vector<bool>& cs, const std::vector<cnoid::Vector3>& ee_pos, const std::vector <cnoid::Matrix3>& ee_rot)
    {
      ...
      for (size_t i = 0; i < cs.size(); i++) {
        if (cs[i]) {
          for (size_t j = 0; j < vs[i].size(); j++) {
            tpos = ee_pos[i] + ee_rot[i] * cnoid::Vector3(vs[i][j](0), vs[i][j](1), 0.0);
    ...
    //
    double calcAlpha (const cnoid::Vector3& tmprefzmp,
                      const std::vector<cnoid::Vector3>& ee_pos,
                      const std::vector<cnoid::Matrix3>& ee_rot,
                      const std::vector<std::string>& ee_name)
    {
        ...
        cnoid::Vector3 l_local_zmp = ee_rot[l_idx].transpose() * (tmprefzmp-ee_pos[l_idx]);
        cnoid::Vector3 r_local_zmp = ee_rot[r_idx].transpose() * (tmprefzmp-ee_pos[r_idx]);
        ...
        if ( is_inside_foot(l_local_zmp, true) && !is_front_of_foot(l_local_zmp) && !is_rear_of_foot(l_local_zmp)) { // new_refzmp is inside lfoot
            ...
            cnoid::Vector3 ledge_foot;
            cnoid::Vector3 redge_foot;
            // lleg
            if (is_inside_foot(l_local_zmp, true) && is_front_of_foot(l_local_zmp)) {
                ledge_foot = cnoid::Vector3(leg_front_margin, l_local_zmp(1), 0.0);
            } else if (!is_inside_foot(l_local_zmp, true) && is_front_of_foot(l_local_zmp)) {
                ledge_foot = cnoid::Vector3(leg_front_margin, -1 * leg_inside_margin, 0.0);
            } else if (!is_inside_foot(l_local_zmp, true) && !is_front_of_foot(l_local_zmp) && !is_rear_of_foot(l_local_zmp)) {
                ledge_foot = cnoid::Vector3(l_local_zmp(0), -1 * leg_inside_margin, 0.0);
            } else if (!is_inside_foot(l_local_zmp, true) && is_rear_of_foot(l_local_zmp)) {
                ledge_foot = cnoid::Vector3(-1 * leg_rear_margin, -1 * leg_inside_margin, 0.0);
            } else {
                ledge_foot = cnoid::Vector3(-1 * leg_rear_margin, l_local_zmp(1), 0.0);
            }
            ledge_foot = ee_rot[l_idx] * ledge_foot + ee_pos[l_idx];
            // rleg
            if (is_inside_foot(r_local_zmp, false) && is_front_of_foot(r_local_zmp)) {
                redge_foot = cnoid::Vector3(leg_front_margin, r_local_zmp(1), 0.0);
            } else if (!is_inside_foot(r_local_zmp, false) && is_front_of_foot(r_local_zmp)) {
                redge_foot = cnoid::Vector3(leg_front_margin, leg_inside_margin, 0.0);
            } else if (!is_inside_foot(r_local_zmp, false) && !is_front_of_foot(r_local_zmp) && !is_rear_of_foot(r_local_zmp)) {
                redge_foot = cnoid::Vector3(r_local_zmp(0), leg_inside_margin, 0.0);
            } else if (!is_inside_foot(r_local_zmp, false) && is_rear_of_foot(r_local_zmp)) {
                redge_foot = cnoid::Vector3(-1 * leg_rear_margin, leg_inside_margin, 0.0);
            } else {
                redge_foot = cnoid::Vector3(-1 * leg_rear_margin, r_local_zmp(1), 0.0);
            }
            redge_foot = ee_rot[r_idx] * redge_foot + ee_pos[r_idx];
            // calc alpha
            cnoid::Vector3 difp = redge_foot - ledge_foot;
    ...
    double calcAlphaFromCOP (const cnoid::Vector3& tmprefzmp,
                             const std::vector<cnoid::Vector3>& cop_pos,
                             const std::vector<std::string>& ee_name)
    {
        size_t l_idx, r_idx;
        for (size_t i = 0; i < ee_name.size(); i++) {
        ...
        cnoid::Vector3 difp = (cop_pos[r_idx]-cop_pos[l_idx]);
    ...
    void calcAlphaVector (std::vector<double>& alpha_vector,
                          ...
                          const std::vector<cnoid::Matrix3>& ee_rot,
                          const std::vector<std::string>& ee_name,
                          const cnoid::Vector3& new_refzmp, const cnoid::Vector3& ref_zmp)
    {
    ...
    void distributeZMPToForceMoments (std::vector<cnoid::Vector3>& ref_foot_force, std::vector<cnoid::Vector3>& ref_foot_moment,
                                      const std::vector<cnoid::Vector3>& ee_pos,
                                      const std::vector<cnoid::Vector3>& cop_pos,
                                      const std::vector<cnoid::Matrix3>& ee_rot,
                                      const std::vector<std::string>& ee_name,
                                      ...
                                      const double total_fz, const double dt, const bool printp = true, const std::string& print_str = "")
    {
        ...
        ref_foot_force[0] = cnoid::Vector3(0,0, fz_alpha_vector[0] * total_fz);
        ref_foot_force[1] = cnoid::Vector3(0,0, fz_alpha_vector[1] * total_fz);

        cnoid::Vector3 tau_0 = cnoid::Vector3::Zero();
#if 0
        ...
        ref_foot_force[0] = cnoid::Vector3(0,0, fz_alpha * total_fz);
        ref_foot_force[1] = cnoid::Vector3(0,0, (1-fz_alpha) * total_fz);
        if (DEBUGP) {
...
//             cnoid::Vector3 foot_dist_coords_y = (cop_pos[1] - cop_pos[0]); // e_y'
//             foot_dist_coords_y(2) = 0.0;
//             foot_dist_coords_y.normalize();
//             cnoid::Vector3 foot_dist_coords_x = cnoid::Vector3(foot_dist_coords_y.cross(cnoid::Vector3::UnitZ())); // e_x'
//             cnoid::Matrix3 foot_dist_coords_rot;
...
//             cnoid::Vector3 tau_0_f = foot_dist_coords_rot.transpose() * tau_0; // tau_0'
//             // x
//             //         // right
...
//             ref_foot_moment[1] = foot_dist_coords_rot * ref_foot_moment[1];
...
#if 0
        {
            ...
            foot_dist_coords_rot(1,2) = 0;
            foot_dist_coords_rot(2,2) = 1;
            tau_0 = cnoid::Vector3::Zero();
            //
            Eigen::VectorXd fvec(3);
            fvec(0) = total_fz;
            fvec(1) = tau_0(0);
            fvec(2) = tau_0(1);
            Eigen::MatrixXd Gmat(3,6);
            ...
            // }
            Eigen::MatrixXd Gmat_ret(6,3);
            hrp::calcSRInverse(Gmat, Gmat_ret, 0.0, Wmat);
            Eigen::VectorXd fmvec(6);
            fmvec = Gmat_ret* fvec;
            ref_foot_force[0] = cnoid::Vector3(0,0,fmvec(0));
            ref_foot_force[1] = cnoid::Vector3(0,0,fmvec(3));
            ref_foot_moment[0] = cnoid::Vector3(fmvec(1),fmvec(2),0);
            ref_foot_moment[1] = cnoid::Vector3(fmvec(4),fmvec(5),0);
            // <= Foot-distribution-coords frame
            // Convert foot-distribution-coords frame => actual world frame
            ref_foot_moment[0] = foot_dist_coords_rot * ref_foot_moment[0];
            ref_foot_moment[1] = foot_dist_coords_rot * ref_foot_moment[1];
        ...
        if (printp) {
            ...
            std::cerr << "[" << print_str << "]   "
                      << "total_tau    = " << cnoid::Vector3(tau_0).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
            std::cerr << "[" << print_str << "]   "
                      << "ref_force_R  = " << cnoid::Vector3(ref_foot_force[0]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N]" << std::endl;
            std::cerr << "[" << print_str << "]   "
                      << "ref_force_L  = " << cnoid::Vector3(ref_foot_force[1]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N]" << std::endl;
            std::cerr << "[" << print_str << "]   "
                      << "ref_moment_R = " << cnoid::Vector3(ref_foot_moment[0]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
            std::cerr << "[" << print_str << "]   "
                      << "ref_moment_L = " << cnoid::Vector3(ref_foot_moment[1]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
        }
    };

#ifdef USE_QPOASES
    void solveForceMomentQPOASES (std::vector<Eigen::VectorXd>& fret,
                                  const size_t state_dim,
                                  const size_t ee_num,
                                  const Eigen::MatrixXd& Hmat,
    ...
    void distributeZMPToForceMomentsQP (std::vector<cnoid::Vector3>& ref_foot_force, std::vector<cnoid::Vector3>& ref_foot_moment,
                                        const std::vector<cnoid::Vector3>& ee_pos,
                                        const std::vector<cnoid::Vector3>& cop_pos,
                                        const std::vector<cnoid::Matrix3>& ee_rot,
                                        const std::vector<std::string>& ee_name,
                                        ...
                                        const bool use_cop_distribution = false)
    {
        ...
        Eigen::VectorXd total_fm(3);
        total_fm(0) = total_fz;
        total_fm(1) = 0;
        total_fm(2) = 0;
        size_t state_dim = 4*ee_num, state_dim_one = 4; // TODO
        //
        std::vector<Eigen::VectorXd> ff(ee_num, Eigen::VectorXd(state_dim_one));
        std::vector<Eigen::MatrixXd> mm(ee_num, Eigen::MatrixXd(3, state_dim_one));
        //
        Eigen::MatrixXd Hmat = Eigen::MatrixXd::Zero(state_dim,state_dim);
        Eigen::VectorXd gvec = Eigen::VectorXd::Zero(state_dim);
        ...
        Eigen::MatrixXd Gmat(3,state_dim);
        for (size_t i = 0; i < state_dim; i++) {
            Gmat(0,i) = 1.0;
        }
        for (size_t fidx = 0; fidx < ee_num; fidx++) {
            for (size_t i = 0; i < state_dim_one; i++) {
                cnoid::Vector3 fpos = ee_rot[fidx]*cnoid::Vector3(fs.get_foot_vertex(fidx,i)(0), fs.get_foot_vertex(fidx,i)(1), 0) + ee_pos[fidx];
                mm[fidx](0,i) = 1.0;
        ...
        //
        {
            Eigen::MatrixXd Kmat = Eigen::MatrixXd::Zero(ee_num,state_dim);
            Eigen::MatrixXd KW = Eigen::MatrixXd::Zero(ee_num, ee_num);
            Eigen::VectorXd reff(ee_num);
        ...
        {
            Eigen::MatrixXd Cmat = Eigen::MatrixXd::Zero(ee_num*2,state_dim);
            Eigen::MatrixXd CW = Eigen::MatrixXd::Zero(ee_num*2,ee_num*2);
            cnoid::Vector3 fpos;
            for (size_t j = 0; j < ee_num; j++) {
                for (size_t i = 0; i < state_dim_one; i++) {
                    fpos = ee_rot[j]*cnoid::Vector3(fs.get_foot_vertex(j,i)(0), fs.get_foot_vertex(j,i)(1), 0) + ee_pos[j];
        ...
        Eigen::VectorXd tmpv(3);
        for (size_t fidx = 0; fidx < ee_num; fidx++) {
            tmpv = mm[fidx] * ff[fidx];
            ref_foot_force[fidx] = cnoid::Vector3(0,0,tmpv(0));
            ref_foot_moment[fidx] = -1*cnoid::Vector3(tmpv(1),tmpv(2),0);
        }
        if (printp) {
            ...
            for (size_t i = 0; i < ee_num; i++) {
                std::cerr << "[" << print_str << "]   "
                          << "ref_force  [" << ee_name[i] << "] " << cnoid::Vector3(ref_foot_force[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N]" << std::endl;
                std::cerr << "[" << print_str << "]   "
                          << "ref_moment [" << ee_name[i] << "] " << cnoid::Vector3(ref_foot_moment[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
            }
        }
    };
#else
    ...
    void calcWeightedLinearEquation(Eigen::VectorXd& ret, const Eigen::MatrixXd& A, const Eigen::MatrixXd& W, const Eigen::VectorXd& b)
    {
        Eigen::MatrixXd W2 = Eigen::MatrixXd::Zero(W.rows(), W.cols());
        for (size_t i = 0; i < W.rows(); i++) W2(i,i) = std::sqrt(W(i,i));
        Eigen::MatrixXd Aw = A*W2;
        // Eigen::MatrixXd Aw_inv = Eigen::MatrixXd::Zero(A.cols(), A.rows());
        ...
        JJ = Aw *  Aw.transpose() + dampingConstantSqr * MatrixXd::Identity(Aw.rows(), Aw.rows());
        ret = W2 * Aw.transpose() * QR.compute(JJ).solve(b);       
    };

    void distributeZMPToForceMomentsPseudoInverse (std::vector<cnoid::Vector3>& ref_foot_force, std::vector<cnoid::Vector3>& ref_foot_moment,
                                                   const std::vector<cnoid::Vector3>& ee_pos,
                                                   const std::vector<cnoid::Vector3>& cop_pos,
                                                   const std::vector<cnoid::Matrix3>& ee_rot,
                                                   const std::vector<std::string>& ee_name,
                                                   ...
                                                   const bool use_cop_distribution = true, const std::vector<bool> is_contact_list = std::vector<bool>())
    {
        ...
        // Temporarily set ref_foot_force and ref_foot_moment based on reference values
        {
            ...
            for (size_t fidx = 0; fidx < ee_num; fidx++) {
                ref_foot_force[fidx] = cnoid::Vector3(ret(3*fidx), ret(3*fidx+1), ret(3*fidx+2));
                ref_foot_moment[fidx] = cnoid::Vector3::Zero();
            }
        ...
        if (printp) {
            for (size_t i = 0; i < ee_num; i++) {
                std::cerr << "[" << print_str << "]   "
                          << "ref_force  [" << ee_name[i] << "] " << cnoid::Vector3(ref_foot_force[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N], "
                          << "ref_moment [" << ee_name[i] << "] " << cnoid::Vector3(ref_foot_moment[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
            }
        }
        ...
        for (size_t fidx = 0; fidx < ee_num; fidx++) {
            ref_foot_force[fidx] += ee_rot[fidx] * cnoid::Vector3(ret(6*fidx), ret(6*fidx+1), ret(6*fidx+2));
            ref_foot_moment[fidx] += ee_rot[fidx] * cnoid::Vector3(ret(6*fidx+3), ret(6*fidx+4), ret(6*fidx+5));
        }
        if (printp) {
            for (size_t i = 0; i < ee_num; i++) {
                std::cerr << "[" << print_str << "]   "
                          << "new_ref_force  [" << ee_name[i] << "] " << cnoid::Vector3(ref_foot_force[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N], "
                          << "new_ref_moment [" << ee_name[i] << "] " << cnoid::Vector3(ref_foot_moment[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
            }
        }
    };

    void distributeZMPToForceMomentsPseudoInverse2 (std::vector<cnoid::Vector3>& ref_foot_force, std::vector<cnoid::Vector3>& ref_foot_moment,
                                                   const std::vector<cnoid::Vector3>& ee_pos,
                                                   const std::vector<cnoid::Vector3>& cop_pos,
                                                   const std::vector<cnoid::Matrix3>& ee_rot,
                                                   const std::vector<std::string>& ee_name,
                                                   ...
                                                   const std::vector<double>& toeheel_ratio,
                                                   const cnoid::Vector3& new_refzmp, const cnoid::Vector3& ref_zmp,
                                                   const cnoid::Vector3& total_force, const cnoid::Vector3& total_moment,
                                                   const std::vector<cnoid::Vector6>& ee_forcemoment_distribution_weight,
                                                   const double total_fz, const double dt, const bool printp = true, const std::string& print_str = "")
    {
#define FORCE_MOMENT_DIFF_CONTROL
        ...
        cnoid::Vector6 total_wrench = Eigen::VectorXd::Zero(6);
        ...
        Eigen::MatrixXd Gmat = Eigen::MatrixXd::Zero(6, state_dim);
        for (size_t j = 0; j < ee_num; j++) {
        ...
        Eigen::MatrixXd selection_matrix = Eigen::MatrixXd::Identity(6,6);
        // Consdier 3DOF total wrench (Fz, Mx, My)
//         Eigen::MatrixXd selection_matrix = Eigen::MatrixXd::Zero(3,6);
//         selection_matrix(0,2) = 1.0;
//         selection_matrix(1,3) = 1.0;
//         selection_matrix(2,4) = 1.0;
        {
            Eigen::VectorXd selected_total_wrench = Eigen::VectorXd::Zero(selection_matrix.rows());
            Eigen::MatrixXd selected_Gmat = Eigen::MatrixXd::Zero(selection_matrix.rows(), Gmat.cols());
        ...
        if (printp) {
            Eigen::VectorXd tmpretv(total_wrench.size());
...
#ifdef FORCE_MOMENT_DIFF_CONTROL
                std::cerr << "[" << print_str << "]   "
                          << "dif_ref_force  [" << ee_name[i] << "] " << cnoid::Vector3(cnoid::Vector3(ret(6*i), ret(6*i+1), ret(6*i+2))).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N], "
                          << "dif_ref_moment [" << ee_name[i] << "] " << cnoid::Vector3(cnoid::Vector3(ret(6*i+3), ret(6*i+4), ret(6*i+5))).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
#else
                std::cerr << "[" << print_str << "]   "
                          << "dif_ref_force  [" << ee_name[i] << "] " << cnoid::Vector3(cnoid::Vector3(ret(6*i), ret(6*i+1), ret(6*i+2))-ref_foot_force[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[N], "
                          << "dif_ref_moment [" << ee_name[i] << "] " << cnoid::Vector3(cnoid::Vector3(ret(6*i+3), ret(6*i+4), ret(6*i+5))-ref_foot_moment[i]).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
#endif
            }
        }
        for (size_t fidx = 0; fidx < ee_num; fidx++) {
#ifdef FORCE_MOMENT_DIFF_CONTROL
            ref_foot_force[fidx] += cnoid::Vector3(ret(6*fidx), ret(6*fidx+1), ret(6*fidx+2));
            ref_foot_moment[fidx] += cnoid::Vector3(ret(6*fidx+3), ret(6*fidx+4), ret(6*fidx+5));
#else
            ref_foot_force[fidx] = cnoid::Vector3(ret(6*fidx), ret(6*fidx+1), ret(6*fidx+2));
            ref_foot_moment[fidx] = cnoid::Vector3(ret(6*fidx+3), ret(6*fidx+4), ret(6*fidx+5));
#endif
