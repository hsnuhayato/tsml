...
std::ostream& operator<<(std::ostream& out, MatrixXd &a) {
    const int c = a.rows();
    const int n = a.cols();
...
std::ostream& operator<<(std::ostream& out, VectorXd &a) {
    const int n = a.size();
...
using namespace std;
using namespace hrp;
int hrp::calcSRInverse(const MatrixXd& _a, MatrixXd &_a_sr, double _sr_ratio, MatrixXd _w) {
    ...
    if ( _w.cols() != n || _w.rows() != n ) {
        _w = MatrixXd::Identity(n, n);
    }

    MatrixXd at = _a.transpose();
    MatrixXd a1(c, c);
    a1 = (_a * _w * at +  _sr_ratio * MatrixXd::Identity(c,c)).inverse();

    //if (DEBUG) { MatrixXd aat = _a * at; std::cerr << " a*at :" << std::endl << aat; }

    _a_sr  = _w * at * a1;
    //if (DEBUG) { MatrixXd ii = _a * _a_sr; std::cerr << "    i :" << std::endl << ii; }
    return 0;
}

// overwrite hrplib/hrpUtil/Eigen3d.cpp
Vector3 omegaFromRotEx(const Matrix3& r)
{
...
bool JointPathEx::calcJacobianInverseNullspace(MatrixXd &J, MatrixXd &Jinv, MatrixXd &Jnull) {
    ...
    for ( int j = 0; j < n ; j++ ) {
        ...
        double jmax = joints[j]->q_upper();
        double jmin = joints[j]->q_lower();
        double e = deg2rad(1);
    ...
    Jnull = ( MatrixXd::Identity(n, n) - Jinv * J);

    return true;
}

bool JointPathEx::calcInverseKinematics2Loop(const Vector3& dp, const Vector3& omega,
                                             const double LAMBDA, const double avoid_gain, const double reference_gain, const VectorXd* reference_q) {
    const int n = numJoints();
    ...
    MatrixXd J(workspace_dim, n);
    VectorXd v(workspace_dim);
    MatrixXd Jinv(n, workspace_dim);
    MatrixXd Jnull(n, n);
    VectorXd dq(n);

    if (ij_workspace_dim > 0) {
        v << dp, omega, VectorXd::Zero(ij_workspace_dim);
        MatrixXd ee_J = MatrixXd::Zero(ee_workspace_dim, n);
        calcJacobian(ee_J);
        MatrixXd ij_J = MatrixXd::Zero(ij_workspace_dim, n);
    ...
    if ( avoid_gain > 0.0 ) {
      ...
      for ( int j = 0; j < n ; j++ ) {
        ...
        double jmax = joint(j)->q_upper();
        double jmin = joint(j)->q_lower();
    ...
    for(int j=0; j < n; ++j){
        ...
        if (dq(j) < joints[j]->dq_lower() * dt) {
            speed_ratio = fabs(joints[j]->dq_lower() * dt / dq(j));
        } else if (dq(j) > joints[j]->dq_upper() * dt) {
            speed_ratio = fabs(joints[j]->dq_upper() * dt / dq(j));
        }
    ...
    for(int j=0; j < n; ++j){
      ...
      if ( joints[j]->q() > joints[j]->q_upper()) {
        is_limit_over = true;
        if (joint_limit_debug_print_counts[j] % debug_print_freq_count == 0) {
            std::cerr << "[" << debug_print_prefix << "] Upper joint limit over " << joints[j]->name()
                      << " (ja=" << joints[j]->q() << "[rad], limit=" << joints[j]->q_upper() << "[rad], count=" << joint_limit_debug_print_counts[j] << ", debug_print_freq_count=" << debug_print_freq_count << ")" << std::endl;
        }
        joints[j]->q() = joints[j]->q_upper();
      }
      if ( joints[j]->q() < joints[j]->q_lower()) {
        is_limit_over = true;
        if (joint_limit_debug_print_counts[j] % debug_print_freq_count == 0) {
            std::cerr << "[" << debug_print_prefix << "] Lower joint limit over " << joints[j]->name()
                      << " (ja=" << joints[j]->q() << "[rad], limit=" << joints[j]->q_lower() << "[rad], count=" << joint_limit_debug_print_counts[j] << ", debug_print_freq_count=" << debug_print_freq_count << ")" << std::endl;
        }
        joints[j]->q() = joints[j]->q_lower();
      }
      joints[j]->q() = std::max(joints[j]->q(), joints[j]->q_lower());
      if (is_limit_over) {
...
// TODO : matrix_logEx should be omegaFromRotEx after checking on real robot testing.
    Vector3 matrix_logEx(const Matrix3& m) {
    Vector3 mlog;
    double q0, th;
    Vector3 q;
    double norm;
...
bool JointPathEx::calcInverseKinematics2Loop(const Vector3& end_effector_p, const Matrix3& end_effector_R,
                                             const double LAMBDA, const double avoid_gain, const double reference_gain, const VectorXd* reference_q,
                                             const double vel_gain,
                                             const Vector3& localPos, const Matrix3& localR)
{
    Matrix3 target_link_R(end_effector_R * localR.transpose());
    Vector3 target_link_p(end_effector_p - target_link_R * localPos);
    Vector3 vel_p(target_link_p - endLink()->p());
    // TODO : matrix_logEx should be omegaFromRotEx after checking on real robot testing.
    //hrp::Vector3 vel_r(endLink()->R() * omegaFromRotEx(endLink()->R().transpose() * target_link_R));
    Vector3 vel_r(endLink()->R() * matrix_logEx(endLink()->R().transpose() * target_link_R));
    vel_p *= vel_gain;
    vel_r *= vel_gain;
    return calcInverseKinematics2Loop(vel_p, vel_r, LAMBDA, avoid_gain, reference_gain, reference_q);
}

bool JointPathEx::calcInverseKinematics2(const Vector3& end_p, const Matrix3& end_R,
                                         const double avoid_gain, const double reference_gain, const VectorXd* reference_q)
{
                                                     ...
                                                     const std::string& instance_name)
{
    ...
    for (unsigned int i=0; i<nvforce; i++){
        ...
        p.localPos = Vector3(tr[0], tr[1], tr[2]);
        p.localR = Eigen::AngleAxis<double>(tr[6], Vector3(tr[3],tr[4],tr[5])).toRotationMatrix(); // rotation in VRML is represented by axis + angle
                                                     ...
                                                     const std::string& instance_name)
{
    ...
    if (interlocking_joints_str.size() > 0) {
        ...
        for (size_t i = 0; i < num; i++) {
            ...
            Link* link1 = m_robot->link(interlocking_joints_str[i*ij_prop_num]);
            Link* link2 = m_robot->link(interlocking_joints_str[i*ij_prop_num+1]);
            ...
            std::pair<Link*, Link*> pair(link1, link2);
            pairs.push_back(pair);
        };
    }
};

void hrp::calcAccelerationsForInverseDynamics(const BodyPtr _m_robot, InvDynStateBuffer& _idsb){
  ...
  _idsb.base_w = Vector3(_idsb.base_w_hat(2,1), - _idsb.base_w_hat(0,2), _idsb.base_w_hat(1,0));
  _idsb.base_dw = (_idsb.base_w - _idsb.base_w_old) / _idsb.DT;
};

// todo
// void hrp::calcRootLinkWrenchFromInverseDynamics(BodyPtr _m_robot, InvDynStateBuffer& _idsb, Vector3& _f_ans, Vector3& _t_ans){
...
// void hrp::calcWorldZMPFromInverseDynamics(BodyPtr _m_robot, InvDynStateBuffer& _idsb, Vector3& _zmp_ans){
//   Vector3 f_tmp, t_tmp;
