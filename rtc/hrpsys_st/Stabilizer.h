  ...
  : public RTC::DataFlowComponentBase
{
  ...
  */
  ...
  void sync_2_st ();
  void sync_2_idle();
  bool calcZMP(cnoid::Vector3& ret_zmp, const double zmp_z);
  ...
  void limbStretchAvoidanceControl (const std::vector<cnoid::Vector3>& ee_p, const std::vector<cnoid::Matrix3>& ee_R);
  ...
  void calcContactMatrix (Eigen::MatrixXd& tm, const std::vector<cnoid::Vector3>& contact_p);
  //void calcTorque ();//unused
  ...
  cnoid::Vector3 calcDampingControl (const cnoid::Vector3& prev_d, const cnoid::Vector3& TT);
  double calcDampingControl (const double prev_d, const double TT);
  cnoid::Vector3 calcDampingControl (const cnoid::Vector3& tau_d, const cnoid::Vector3& tau, const cnoid::Vector3& prev_d,
                                   const cnoid::Vector3& DD, const cnoid::Vector3& TT);
  double vlimit(double value, double llimit_value, double ulimit_value);
  cnoid::Vector3 vlimit(const cnoid::Vector3& value, double llimit_value, double ulimit_value);
  cnoid::Vector3 vlimit(const cnoid::Vector3& value, const cnoid::Vector3& limit_value);
  cnoid::Vector3 vlimit(const cnoid::Vector3& value, const cnoid::Vector3& llimit_value, const cnoid::Vector3& ulimit_value);
 ...
 private:
  ...
  struct STIKParam {
    ...
    cnoid::Vector3 localp; // Position of ee in end link frame (^{l}p_e = R_l^T (p_e - p_l))
    cnoid::Vector3 localCOPPos; // Position offset of reference COP in end link frame (^{l}p_{cop} = R_l^T (p_{cop} - p_l) - ^{l}p_e)
    cnoid::Matrix3 localR; // Rotation of ee in end link frame (^{l}R_e = R_l^T R_e)
    // For eefm
    cnoid::Vector3 d_foot_pos, ee_d_foot_pos, d_foot_rpy, ee_d_foot_rpy;
    cnoid::Vector3 eefm_pos_damping_gain, eefm_pos_time_const_support, eefm_rot_damping_gain, eefm_rot_time_const, eefm_swing_rot_spring_gain, eefm_swing_pos_spring_gain, eefm_swing_rot_time_const, eefm_swing_pos_time_const, eefm_ee_moment_limit;
    double eefm_pos_compensation_limit, eefm_rot_compensation_limit;
    cnoid::Vector3 ref_force, ref_moment;
    cnoid::Vector6 eefm_ee_forcemoment_distribution_weight;
    double swing_support_gain, support_time;
    // For swing ee modification
    boost::shared_ptr<FirstOrderLowPassFilter<cnoid::Vector3> > target_ee_diff_p_filter, target_ee_diff_r_filter;
    cnoid::Vector3 target_ee_diff_p, d_pos_swing, d_rpy_swing, prev_d_pos_swing, prev_d_rpy_swing;
    cnoid::Matrix3 target_ee_diff_r;
    // IK parameter
  ...
  cnoid::DeviceList<cnoid::ForceSensor> forceSensors;
  cnoid::DeviceList<cnoid::RateGyroSensor> RateGyroSensors;
  coil::Mutex m_mutex;
  unsigned int m_debugLevel;
  Eigen::VectorXd transition_joint_q, qorg, qrefv;
  ...
  cnoid::Vector3 current_root_p, target_root_p;
  cnoid::Matrix3 current_root_R, target_root_R, prev_act_foot_origin_rot, prev_ref_foot_origin_rot, target_foot_origin_rot, ref_foot_origin_rot;
  std::vector <cnoid::Vector3> target_ee_p, rel_ee_pos, act_ee_p, projected_normal, act_force, ref_force, ref_moment;
  std::vector <cnoid::Matrix3> target_ee_R, rel_ee_rot, act_ee_R;
  std::vector<std::string> rel_ee_name;
  rats::coordinates target_foot_midcoords;
  cnoid::Vector3 ref_zmp, ref_cog, ref_cp, ref_cogvel, rel_ref_cp, prev_ref_cog, prev_ref_zmp;
  cnoid::Vector3 act_zmp, act_cog, act_cogvel, act_cp, rel_act_zmp, rel_act_cp, prev_act_cog, act_base_rpy, current_base_rpy, current_base_pos, sbp_cog_offset, cp_offset, diff_cp;
  cnoid::Vector3 foot_origin_offset[2];
  ...
  cnoid::Vector3 new_refzmp, rel_cog, ref_zmp_aux, diff_foot_origin_ext_moment;
  cnoid::Vector3 pos_ctrl;
  cnoid::Vector3 ref_total_force, ref_total_moment;
  // all reference wrench is given from other rtc
  bool is_ref_wrench_given;
  // Total foot moment around the foot origin coords (relative to foot origin coords)
  cnoid::Vector3 ref_total_foot_origin_moment, act_total_foot_origin_moment;
  cnoid::Vector3 eefm_swing_pos_damping_gain, eefm_swing_rot_damping_gain;
