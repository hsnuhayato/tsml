...
static const char* stabilizer_spec[] =
  {
    ...
    "version",           "HRPSYS_PACKAGE_VERSION", //get rid of hrpsys
    "vendor",            "AIST",
    "category",          "example",
...
RTC::ReturnCode_t Stabilizer::onInitialize()
{
  ...
  int npforce = forceSensors.size();
  for (unsigned int i=0; i<npforce; ++i) {
    force_sensor_names.push_back(forceSensors[i]->name());
    ...
    for (size_t i = 0; i < num; i++) {
      ...
      // for find sensor name
      {
          ...
          for (size_t j = 0; j < npforce; j++) {
              ...
              cnoid::Link* alink = m_robot->link(ikp.target_name);
              while (alink != NULL && alink->name() != ee_base && !is_ee_exists) {
                if ( alink -> name() == forceSensors[i]->link()->name() ) {
                      is_ee_exists = true;
                      ikp.sensor_name = forceSensors[i]->name();
                  }
                  alink = alink->parent();
              }
          }
      ...
      ikp.target_ee_diff_p = cnoid::Vector3::Zero();
      ikp.target_ee_diff_r = cnoid::Matrix3::Identity();
      ikp.d_rpy_swing = cnoid::Vector3::Zero();
      ikp.d_pos_swing = cnoid::Vector3::Zero();
      ikp.target_ee_diff_p_filter = boost::shared_ptr<FirstOrderLowPassFilter<cnoid::Vector3> >(new FirstOrderLowPassFilter<cnoid::Vector3>(50.0, dt, cnoid::Vector3::Zero())); // [Hz]
      ikp.target_ee_diff_r_filter = boost::shared_ptr<FirstOrderLowPassFilter<cnoid::Vector3> >(new FirstOrderLowPassFilter<cnoid::Vector3>(50.0, dt, cnoid::Vector3::Zero())); // [Hz]
      ikp.prev_d_pos_swing = cnoid::Vector3::Zero();
      ikp.prev_d_rpy_swing = cnoid::Vector3::Zero();
      //
      stikp.push_back(ikp);
      ...
      target_ee_p.push_back(cnoid::Vector3::Zero());
      target_ee_R.push_back(cnoid::Matrix3::Identity());
      act_ee_p.push_back(cnoid::Vector3::Zero());
      act_ee_R.push_back(cnoid::Matrix3::Identity());
      projected_normal.push_back(cnoid::Vector3::Zero());
      act_force.push_back(cnoid::Vector3::Zero());
      ref_force.push_back(cnoid::Vector3::Zero());
      ref_moment.push_back(cnoid::Vector3::Zero());
  ...
  hrp::readInterlockingJointsParamFromProperties(interlocking_joints, m_robot, prop["interlocking_joints"], std::string(m_profile.instance_name));
  ...
  for (size_t i = 0; i < stikp.size(); i++) {
      ...
      cnoid::Link* root = m_robot->link(ikp.target_name);
      ikp.eefm_rot_damping_gain = cnoid::Vector3(20*5, 20*5, 1e5);
      ikp.eefm_rot_time_const = cnoid::Vector3(1.5, 1.5, 1.5);
      ikp.eefm_rot_compensation_limit = deg2rad(10.0);
      ikp.eefm_swing_rot_spring_gain = cnoid::Vector3(0.0, 0.0, 0.0);
      ikp.eefm_swing_rot_time_const = cnoid::Vector3(1.5, 1.5, 1.5);
      ikp.eefm_pos_damping_gain = cnoid::Vector3(3500*10, 3500*10, 3500);
      ikp.eefm_pos_time_const_support = cnoid::Vector3(1.5, 1.5, 1.5);
      ikp.eefm_pos_compensation_limit = 0.025;
      ikp.eefm_swing_pos_spring_gain = cnoid::Vector3(0.0, 0.0, 0.0);
      ikp.eefm_swing_pos_time_const = cnoid::Vector3(1.5, 1.5, 1.5);
      ikp.eefm_ee_moment_limit = cnoid::Vector3(1e4, 1e4, 1e4); // Default limit [Nm] is too large. Same as no limit.
  ...
  cp_offset = cnoid::Vector3(0.0, 0.0, 0.0); // [m]
  ...
  sbp_cog_offset = cnoid::Vector3(0.0, 0.0, 0.0);
  ...
  for (size_t i = 0; i < stikp.size(); i++) {
      ...
      // if ( sen != NULL ) is_legged_robot = trua;
      // @todo get sensor reference from sensor name.
  ...
  ref_zmp_aux = cnoid::Vector3::Zero();
  ...
  transition_time = 2.0;
  foot_origin_offset[0] = cnoid::Vector3::Zero();
  foot_origin_offset[1] = cnoid::Vector3::Zero();

  //
  act_cogvel_filter = boost::shared_ptr<FirstOrderLowPassFilter<cnoid::Vector3> >(new FirstOrderLowPassFilter<cnoid::Vector3>(4.0, dt, cnoid::Vector3::Zero())); // [Hz]
  ...
  prev_act_cog = cnoid::Vector3::Zero();
  //prev_ref_cog = prev_ref_zmp = cnoid::Vector3::Zero();
  return RTC::RTC_OK;
}
...
RTC::ReturnCode_t Stabilizer::onExecute(RTC::UniqueId ec_id)
{
  ...
  for (size_t i = 0; i < m_limbCOPOffsetIn.size(); ++i) {
    if ( m_limbCOPOffsetIn[i]->isNew() ) {
      ...
      //stikp[i].localCOPPos = stikp[i].localp + stikp[i].localR * cnoid::Vector3(m_limbCOPOffset[i].data.x, m_limbCOPOffset[i].data.y, m_limbCOPOffset[i].data.z);
      stikp[i].localCOPPos = stikp[i].localp + stikp[i].localR * cnoid::Vector3(m_limbCOPOffset[i].data.x, 0, m_limbCOPOffset[i].data.z);
    }
  }
  ...
  if ( m_robot->numJoints() == m_qRef.data.length() ) {
    if (is_legged_robot) {
      ...
      m_actCPOut.write();
      {
        cnoid::Vector3 tmp_diff_cp = ref_foot_origin_rot * (ref_cp - act_cp - cp_offset);
...
void Stabilizer::calcFootOriginCoords (cnoid::Vector3& foot_origin_pos, cnoid::Matrix3& foot_origin_rot)
{
  ...
  for (size_t i = 0; i < stikp.size(); i++) {
    if (stikp[i].ee_name.find("leg") == std::string::npos) continue;
    //cnoid::Link* target = m_robot->sensor<hrp::ForceSensor>(stikp[i].sensor_name)->link;
    cnoid::Link* target = forceSensors[i]->link();
    // caution
    ...
    xv1(2)=0.0;
    xv1.normalize();
    cnoid::Vector3 yv1(ez.cross(xv1));
...
void Stabilizer::getActualParameters ()
{
  ...
  on_ground = false;
  if (st_algorithm != OpenHRP::StabilizerService::TPCC) {
    // second arg:zmp height in world frame(root link p=[0,0,0])
  ...
  if (st_algorithm != OpenHRP::StabilizerService::TPCC) {
    ...
    for (size_t i = 0; i < stikp.size(); i++) {
      ...
      //cnoid::Vector3 act_ee_p = target->p + target->R * stikp[i].localCOPPos;
      cnoid::Vector3 _act_ee_p = target->p() + target->R() * stikp[i].localp;
    ...
    rel_act_cp = cnoid::Vector3(act_cp(0), act_cp(1), act_zmp(2));
    ...
    if (DEBUGP) {
      ...
      std::cerr << "[" << m_profile.instance_name << "]   "
                << "ref_cog    = " << cnoid::Vector3(ref_cog*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"))
                << ", act_cog    = " << cnoid::Vector3(act_cog*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[mm]" << std::endl;
      std::cerr << "[" << m_profile.instance_name << "]   "
                << "ref_cogvel = " << cnoid::Vector3(ref_cogvel*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"))
                << ", act_cogvel = " << cnoid::Vector3(act_cogvel*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[mm/s]" << std::endl;
      std::cerr << "[" << m_profile.instance_name << "]   "
                << "ref_zmp    = " << cnoid::Vector3(ref_zmp*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"))
                << ", act_zmp    = " << cnoid::Vector3(act_zmp*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[mm]" << std::endl;
      cnoid::Vector3 tmpnew_refzmp;
    ...
    std::vector<cnoid::Vector3> tmp_ref_force, tmp_ref_moment;
    std::vector<double> limb_gains;
    std::vector<cnoid::Vector6> ee_forcemoment_distribution_weight;
    std::vector<double> tmp_toeheel_ratio;
    if (control_mode == MODE_ST) {
      std::vector<cnoid::Vector3> ee_pos, cop_pos;
      std::vector<cnoid::Matrix3> ee_rot;
      ...
      for (size_t i = 0; i < stikp.size(); i++) {
          ...
          ee_forcemoment_distribution_weight.push_back(cnoid::Vector6::Zero());
          for (size_t j = 0; j < 6; j++) {
            //ikp.eefm_ee_forcemoment_distribution_weight=[1,1,1,1e-2,1e-2,1e-2]
      ...
      if (DEBUGP) {
          ...
          for (size_t i = 0; i < ee_name.size(); i++) {
              ...
              std::cerr << "[" << m_profile.instance_name << "]   "
                        << "ee_pos (" << ee_name[i] << ")    = " << cnoid::Vector3(tmpp*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"));
              tmpp = foot_origin_rot.transpose()*(cop_pos[i]-foot_origin_pos);
              std::cerr << ", cop_pos (" << ee_name[i] << ")    = " << cnoid::Vector3(tmpp*1e3).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]")) << "[mm]" << std::endl;
          }
      }
      ...
      if (use_zmp_truncation) {
        ...
        if (!szd->is_inside_support_polygon(tmp_new_refzmp, cnoid::Vector3::Zero(), true, std::string(m_profile.instance_name))) new_refzmp.head(2) = tmp_new_refzmp;
      }
      ...
      if (st_algorithm == OpenHRP::StabilizerService::EEFM) {
        ...
        // arg: ref wrench in foot frame, ee in world frame,  limb_gains= 1, tmp_toeheel_ratio =1
    ...
    // foor modif
    if (control_mode == MODE_ST) {
      cnoid::Vector3 f_diff(cnoid::Vector3::Zero());
      std::vector<bool> large_swing_f_diff(3, false);
      // moment control
      act_total_foot_origin_moment = cnoid::Vector3::Zero();
      for (size_t i = 0; i < stikp.size(); i++) {
        STIKParam& ikp = stikp[i];
        ...
        //cnoid::Vector3 ee_moment = ((sensor->link->R * sensor->localPos + sensor->link->p) - (target->R * ikp.localCOPPos + target->p)).cross(sensor_force) + sensor_moment;
        cnoid::Vector3 ee_moment = ((sensor->link()->R() * sensor->p_local() + sensor->link()->p()) - (target->R() * ikp.localp + target->p())).cross(sensor_force) + sensor_moment;
        // <= Actual world frame
        cnoid::Matrix3 ee_R(target->R() * ikp.localR);
        // Actual ee frame =>
        ...
        // Moment limitation (bug? twice multipled by ee_R.transpose())
        // maybe  ikp.ref_moment = vlimit(ikp.ref_moment, ikp.eefm_ee_moment_limit); is fine
        ...
        { // Rot
          ...
          cnoid::Vector3 tmp_damping_gain;
          for (size_t j = 0; j < 3; ++j) {
        ...
        if (!eefm_use_force_difference_control) { // Pos
            cnoid::Vector3 tmp_damping_gain = (1-transition_smooth_gain) * ikp.eefm_pos_damping_gain * 10 + transition_smooth_gain * ikp.eefm_pos_damping_gain;
        ...
        {
            cnoid::Vector3 plane_x = target_ee_R[i].col(0);
            cnoid::Vector3 plane_y = target_ee_R[i].col(1);
            cnoid::Matrix3 act_ee_R_world = target->R() * stikp[i].localR;
            cnoid::Vector3 normal_vector = act_ee_R_world.col(2);
      ...
      if (eefm_use_force_difference_control) {
          ...
          cnoid::Vector3 ref_f_diff = (stikp[1].ref_force-stikp[0].ref_force);
          ...
          if (eefm_use_swing_damping) {
            ...
            if ( (ref_contact_states[contact_states_index_map["rleg"]] && ref_contact_states[contact_states_index_map["lleg"]]) // Reference : double support phase
              ...
              // std::cerr << "st " << remain_swing_time << " rleg " << contact_states[contact_states_index_map["rleg"]] << " lleg " << contact_states[contact_states_index_map["lleg"]] << std::endl; eefm_pos_margin_time = 0.02
  ...
  for ( int i = 0; i < m_robot->numJoints(); i++ ){
    m_robot->joint(i)->q() = qrefv[i];// angles from refq
  }
...
}// stikp[i].d_foot_pos d_foot_pos, stikp[i].ee_d_foot_rpy ready for IK
...
void Stabilizer::getTargetParameters ()
{
  ...
  for ( int i = 0; i < m_robot->numJoints(); i++ ){
    qrefv[i] = m_robot->joint(i)->q(); // m_qRef.data[i];
  }
  // assign reference subjects into robot model
  m_robot->rootLink()->p() = cnoid::Vector3(m_basePos.data.x, m_basePos.data.y, m_basePos.data.z);
  target_root_p = m_robot->rootLink()->p();
  target_root_R = cnoid::rotFromRpy(m_baseRpy.data.r, m_baseRpy.data.p, m_baseRpy.data.y);
  m_robot->rootLink()->R() = target_root_R;
  m_robot->calcForwardKinematics();
  ref_zmp = m_robot->rootLink()->R() * cnoid::Vector3(m_zmpRef.data.x, m_zmpRef.data.y, m_zmpRef.data.z) + m_robot->rootLink()->p(); // base frame -> world frame
  // ref_zmp_world = ref_zmp
  cnoid::Vector3 foot_origin_pos;
  cnoid::Matrix3 foot_origin_rot;
  calcFootOriginCoords (foot_origin_pos, foot_origin_rot);
  if (st_algorithm != OpenHRP::StabilizerService::TPCC) {
    // apply inverse system
    cnoid::Vector3 tmp_ref_zmp = ref_zmp + eefm_zmp_delay_time_const[0] * (ref_zmp - prev_ref_zmp) / dt;
  ...
  for (size_t i = 0; i < stikp.size(); i++) {
    ...
    ref_force[i] = cnoid::Vector3(m_ref_wrenches[i].data[0], m_ref_wrenches[i].data[1], m_ref_wrenches[i].data[2]);
    ref_moment[i] = cnoid::Vector3(m_ref_wrenches[i].data[3], m_ref_wrenches[i].data[4], m_ref_wrenches[i].data[5]);
    ...
    // ref_total_moment += (target_ee_p[i]-ref_zmp).cross(cnoid::Vector3(m_ref_wrenches[i].data[0], m_ref_wrenches[i].data[1], m_ref_wrenches[i].data[2]))
    //     + cnoid::Vector3(m_ref_wrenches[i].data[3], m_ref_wrenches[i].data[4], m_ref_wrenches[i].data[5]);
    if (is_feedback_control_enable[i]) {
      ref_total_foot_origin_moment += (target_ee_p[i]-foot_origin_pos).cross(ref_force[i]) + ref_moment[i];//moment around foot origin 
    }
  }
  // <= Reference world frame
  ...
  if (st_algorithm != OpenHRP::StabilizerService::TPCC) {
    ...
    target_foot_origin_rot = foot_origin_rot; //unused
    // capture point
    ref_cp = ref_cog + ref_cogvel / std::sqrt(eefm_gravitational_acceleration / (ref_cog - ref_zmp)(2));
    rel_ref_cp = cnoid::Vector3(ref_cp(0), ref_cp(1), ref_zmp(2));
...
bool Stabilizer::calcZMP(cnoid::Vector3& ret_zmp, const double zmp_z)
{
  double tmpzmpx = 0;
  double tmpzmpy = 0;
  ...
  for (size_t i = 0; i < stikp.size(); i++) {
    ...
    cnoid::Vector3 nf = tmpR * cnoid::Vector3(m_wrenches[i].data[0], m_wrenches[i].data[1], m_wrenches[i].data[2]);
    cnoid::Vector3 nm = tmpR * cnoid::Vector3(m_wrenches[i].data[3], m_wrenches[i].data[4], m_wrenches[i].data[5]);
    tmpzmpx += nf(2) * fsp(0) - (fsp(2) - zmp_z) * nf(0) - nm(1);
    tmpzmpy += nf(2) * fsp(1) - (fsp(2) - zmp_z) * nf(1) + nm(0);
    tmpfz += nf(2);
    // calc ee-local COP
    cnoid::Link* target = m_robot->link(stikp[i].target_name);
    cnoid::Matrix3 eeR = target->R() * stikp[i].localR;
    cnoid::Vector3 ee_fsp = eeR.transpose() * (fsp - (target->p() + target->R() * stikp[i].localp)); // ee-local force sensor pos
    nf = eeR.transpose() * nf;
    nm = eeR.transpose() * nm;
...
void Stabilizer::calcStateForEmergencySignal()
{
  ...
  if (on_ground && transition_count == 0 && control_mode == MODE_ST) {
    ...
    for (size_t i = 0; i < stikp.size(); i++) {
      ...
      if (m_COPInfo.data[i*3+2] > 20.0 ) {
        cnoid::Vector3 tmpcop(m_COPInfo.data[i*3+1]/m_COPInfo.data[i*3+2], m_COPInfo.data[i*3]/m_COPInfo.data[i*3+2], 0);
  ...
  // tilt Check
  cnoid::Vector3 fall_direction = cnoid::Vector3::Zero();
  bool is_falling = false, will_fall = false;
  {
      ...
      if (on_ground && transition_count == 0 && control_mode == MODE_ST) {
          ...
          fall_direction = cnoid::Vector3::Zero();
      }
...
void Stabilizer::moveBasePosRotForBodyRPYControl ()
{
    ...
    cnoid::Vector3 ref_root_rpy = cnoid::rpyFromRot(target_root_R);
    ...
    rats::rotm3times(current_root_R, target_root_R, cnoid::rotFromRpy(d_rpy[0], d_rpy[1], 0));
    m_robot->rootLink()->R() = current_root_R;
    m_robot->rootLink()->p() = target_root_p + target_root_R * rel_cog - current_root_R * rel_cog;
    m_robot->calcForwardKinematics();
    current_base_rpy = cnoid::rpyFromRot(m_robot->rootLink()->R());
...
void Stabilizer::calcSwingSupportLimbGain ()
{
    for (size_t i = 0; i < stikp.size(); i++) {
        ...
        if (ref_contact_states[i]) { // Support
            ...
            // default is 0.5, 1, m_controlSwingSupportTime.data[i]*100,
...
void Stabilizer::calcTPCC() {
    // stabilizer loop
      // Choi's feedback law
      cnoid::Vector3 cog = m_robot->calcCenterOfMass();
      cnoid::Vector3 newcog = cnoid::Vector3::Zero();
      cnoid::Vector3 dcog(ref_cog - act_cog);
      cnoid::Vector3 dzmp(ref_zmp - act_zmp);
      for (size_t i = 0; i < 2; i++) {
      ...
      cnoid::Vector3 target_link_p[stikp.size()];
      cnoid::Matrix3 target_link_R[stikp.size()];
      for (size_t i = 0; i < stikp.size(); i++) {
      ...
      for (size_t jj = 0; jj < max_ik_loop_count; jj++) {
        cnoid::Vector3 tmpcm = m_robot->calcCenterOfMass();
        for (size_t i = 0; i < 2; i++) {
...
void Stabilizer::calcEEForceMomentControl() {
      ...
      //   root rot : actual root rot
      {
          ...
          // target_ee_diff_xx = target_ee pose in ref_foot_coord
          for (size_t i = 0; i < stikp.size(); i++) {
              cnoid::Link* target = m_robot->link(stikp[i].target_name);
      ...
      // calculate current_base_rpy current_base_pos
      moveBasePosRotForBodyRPYControl();
      ...
      cnoid::Matrix3 foot_origin_rot;
      calcFootOriginCoords (foot_origin_pos, foot_origin_rot);
      std::vector<cnoid::Vector3> current_d_foot_pos;
      for (size_t i = 0; i < stikp.size(); i++)
      ...
      // solveIK
      //   IK target is link origin pos and rot, not ee pos and rot.
      std::vector<cnoid::Vector3> tmpp(stikp.size());
      std::vector<cnoid::Matrix3> tmpR(stikp.size());
      double tmp_d_pos_z_root = 0.0;
      for (size_t i = 0; i < stikp.size(); i++) {
        if (is_ik_enable[i]) {
          // Add damping_control compensation to target value
          if (is_feedback_control_enable[i]) {
            rats::rotm3times(tmpR[i], target_ee_R[i], cnoid::rotFromRpy(-1*stikp[i].ee_d_foot_rpy));
            // foot force difference control version
          ...
          rats::rotm3times(tmpR[i], tmpR[i], cnoid::rotFromRpy(stikp[i].d_rpy_swing));
...
void Stabilizer::calcSwingEEModification ()
{
    for (size_t i = 0; i < stikp.size(); i++) {
        ...
        if (ref_contact_states[contact_states_index_map[stikp[i].ee_name]] || act_contact_states[contact_states_index_map[stikp[i].ee_name]]) {
            ...
            /* position */
            {
                cnoid::Vector3 tmpdiffp = stikp[i].eefm_swing_pos_spring_gain.cwiseProduct(stikp[i].target_ee_diff_p_filter->passFilter(stikp[i].target_ee_diff_p));
                double lvlimit = -50 * 1e-3 * dt, uvlimit = 50 * 1e-3 * dt; // 50 [mm/s]
                cnoid::Vector3 limit_by_lvlimit = stikp[i].prev_d_pos_swing + lvlimit * cnoid::Vector3::Ones();
                cnoid::Vector3 limit_by_uvlimit = stikp[i].prev_d_pos_swing + uvlimit * cnoid::Vector3::Ones();
                stikp[i].d_pos_swing = vlimit(vlimit(tmpdiffp, -1 * limit_pos, limit_pos), limit_by_lvlimit, limit_by_uvlimit);
            }
            /* rotation */
            {
                cnoid::Vector3 tmpdiffr = stikp[i].eefm_swing_rot_spring_gain.cwiseProduct(stikp[i].target_ee_diff_r_filter->passFilter(cnoid::rpyFromRot(stikp[i].target_ee_diff_r)));
                double lvlimit = deg2rad(-20.0*dt), uvlimit = deg2rad(20.0*dt); // 20 [deg/s]
                cnoid::Vector3 limit_by_lvlimit = stikp[i].prev_d_rpy_swing + lvlimit * cnoid::Vector3::Ones();
                cnoid::Vector3 limit_by_uvlimit = stikp[i].prev_d_rpy_swing + uvlimit * cnoid::Vector3::Ones();
...
void Stabilizer::limbStretchAvoidanceControl (const std::vector<cnoid::Vector3>& ee_p, const std::vector<cnoid::Matrix3>& ee_R)
{
  ...
  if (use_limb_stretch_avoidance) {
    for (size_t i = 0; i < stikp.size(); i++) {
      if (is_ik_enable[i]) {
        ...
        cnoid::Link* parent_link = m_robot->link(stikp[i].parent_name);
        cnoid::Vector3 targetp = (ee_p[i] - ee_R[i] * stikp[i].localR.transpose() * stikp[i].localp) - parent_link->p(); // position from parent to target link (world frame)
...
// Retrieving only
cnoid::Vector3 Stabilizer::calcDampingControl (const cnoid::Vector3& prev_d, const cnoid::Vector3& TT)
{
...
cnoid::Vector3 Stabilizer::calcDampingControl (const cnoid::Vector3& tau_d, const cnoid::Vector3& tau, const cnoid::Vector3& prev_d,
                                             const cnoid::Vector3& DD, const cnoid::Vector3& TT)
{
...
void Stabilizer::calcDiffFootOriginExtMoment ()
{
    ...
    cnoid::Vector3 ref_ext_moment = cnoid::Vector3(mg * ref_cog(1) - ref_total_foot_origin_moment(0),
                                               -mg * ref_cog(0) - ref_total_foot_origin_moment(1),
                                               0);
    // calc act ext moment around foot origin pos
    cnoid::Vector3 act_ext_moment = cnoid::Vector3(mg * act_cog(1) - act_total_foot_origin_moment(0),
...
void Stabilizer::sync_2_st ()
{
  ...
  d_rpy[0] = d_rpy[1] = 0;
  pdr = cnoid::Vector3::Zero();
  pos_ctrl = cnoid::Vector3::Zero();
  for (size_t i = 0; i < stikp.size(); i++) {
    STIKParam& ikp = stikp[i];
    ikp.target_ee_diff_p = cnoid::Vector3::Zero();
    ikp.target_ee_diff_r = cnoid::Matrix3::Identity();
    ikp.d_pos_swing = ikp.prev_d_pos_swing = cnoid::Vector3::Zero();
    ikp.d_rpy_swing = ikp.prev_d_rpy_swing = cnoid::Vector3::Zero();
    ikp.target_ee_diff_p_filter->reset(cnoid::Vector3::Zero());
...
cnoid::Vector3 Stabilizer::vlimit(const cnoid::Vector3& value, double llimit_value, double ulimit_value)
{
  cnoid::Vector3 ret;
  for (size_t i = 0; i < 3; i++) {
...
cnoid::Vector3 Stabilizer::vlimit(const cnoid::Vector3& value, const cnoid::Vector3& limit_value)
{
  cnoid::Vector3 ret;
  for (size_t i = 0; i < 3; i++) {
...
cnoid::Vector3 Stabilizer::vlimit(const cnoid::Vector3& value, const cnoid::Vector3& llimit_value, const cnoid::Vector3& ulimit_value)
{
  cnoid::Vector3 ret;
  for (size_t i = 0; i < 3; i++) {
...
void Stabilizer::calcRUNST() {
  if ( m_robot->numJoints() == m_qRef.data.length() ) {
    ...
    //set root
    m_robot->rootLink()->p() = cnoid::Vector3(0,0,0);
    //m_robot->rootLink()->R() = hrp::rotFromRpy(m_rpyRef.data.r,m_rpyRef.data.p,m_rpyRef.data.y);
    m_robot->calcForwardKinematics();
    cnoid::Vector3 target_root_p = m_robot->rootLink()->p();
    cnoid::Matrix3 target_root_R = m_robot->rootLink()->R();
    cnoid::Vector3 target_foot_p[2];
    cnoid::Matrix3 target_foot_R[2];
    for (size_t i = 0; i < 2; i++) {
      target_foot_p[i] = m_robot->link(target_name[i])->p();
      target_foot_R[i] = m_robot->link(target_name[i])->R();
    }
    cnoid::Vector3 target_fm = (m_robot->link(target_name[0])->p() + m_robot->link(target_name[1])->p())/2;
    //cnoid::Vector3 org_cm = m_robot->rootLink()->R().transpose() * (m_robot->calcCenterOfMass() - m_robot->rootLink()->p());
    cnoid::Vector3 org_cm = m_robot->rootLink()->R().transpose() * (target_fm - m_robot->rootLink()->p());

    // stabilizer loop
    if ( ( m_wrenches[1].data.length() > 0 && m_wrenches[0].data.length() > 0 )
         //( m_wrenches[ST_LEFT].data[2] > m_robot->mass()/4 || m_wrenches[ST_RIGHT].data[2] > m_robot->mass()/4 )
         ) {
      ...
      // }
      cnoid::Vector3 root_p_s;
      cnoid::Matrix3 root_R_s;
      rats::rotm3times(root_R_s, cnoid::rotFromRpy(rx, ry, 0), target_root_R);
      if (DEBUGP2) {
        cnoid::Vector3 tmp = cnoid::rpyFromRot(root_R_s);
      ...
      m_robot->calcForwardKinematics();
      //
      cnoid::Vector3 current_fm = (m_robot->link(target_name[0])->p() + m_robot->link(target_name[1])->p())/2;

      // 3D-LIP model contorller
      cnoid::Vector3 dr = target_fm - current_fm;
      //cnoid::Vector3 dr = current_fm - target_fm ;
      cnoid::Vector3 dr_vel = (dr - pdr)/dt;
      pdr = dr;
      ...
      double yin = 0.02;
      double yout = 0.15;
      double mg = m_robot -> mass() * 9.8 * 0.9;// margin
      ...
      // calc leg rot
      cnoid::Matrix3 target_R[2];
      cnoid::Vector3 target_p[2];
      for (size_t i = 0; i < 2; i++) {
        //rats::rotm3times(target_R[i], hrp::rotFromRpy(dleg_x[i], dleg_y[i], 0), target_foot_R[i]);
        rats::rotm3times(target_R[i], cnoid::rotFromRpy(0, dleg_y[i], 0), target_foot_R[i]);
      ...
      // IK
      for (size_t i = 0; i < 2; i++) {
        cnoid::Link* target = m_robot->link(target_name[i]);
        cnoid::Vector3 vel_p, vel_r;
...
void Stabilizer::calcContactMatrix (Eigen::MatrixXd& tm, const std::vector<cnoid::Vector3>& contact_p)
{
...
//   cnoid::Vector3 root_w_x_v;
//   cnoid::Vector3 g(0, 0, 9.80665);
...
//     hrp::JointPathEx jm = hrp::JointPathEx(m_robot, m_robot->rootLink(), forceSensors[i]->link(), dt);
//     Eigen::MatrixXd JJ;
//     jm.calcJacobian(JJ);
//     Eigen::VectorXd ft(6);
//     for (size_t i = 0; i < 6; i++) ft(i) = contact_ft(i+j*6);
//     Eigen::VectorXd tq_from_extft(jm.numJoints());
...
//   }
//   //Eigen::MatrixXd MM(6,m_robot->numJoints());
...
//   //   std::cerr << ")" << std::endl;
//   //   // Eigen::VectorXd tqv(m_robot->numJoints());
